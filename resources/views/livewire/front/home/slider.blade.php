                    ...
                    <div class="swiper-slide relative">
                        <div class="absolute flex w-full h-full justify-center items-center">
                            <div class="relative w-full h-full overflow-hidden">
                                <img class="w-full h-full object-center object-contain md:object-cover" src="{{ url($data->image) }}" alt="">
                            </div>
                        </div>

                        <div class="absolute bottom-10 w-full flex flex-col gap-2 items-center z-30">
                            <div class="bg-transparent hover:bg-accent drop-shadow-md rounded-full p-3 transition-all duration-300">
                                <button onclick="handleLike('{{ $data->id }}')" id="like-button-{{ $data->id }}">
                                    @if (in_array($data->id, $likedSliders))
                                        <i class="fa-solid fa-heart text-4xl text-red-500"></i>
                                    @else
